...
import { getLeaveCalendar, downloadLeavePdf, type LeaveRequest } from '../services/api';
...
const LeaveCalendar = () => {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      const response = await downloadLeavePdf(month, year);
      const blob = new Blob([response.data], { type: 'application/pdf' });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `leave_report_${month}_${year}.pdf`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
    } catch (error) {
      alert('فشل تحميل تقرير الإجازات. قد يكون الملحق قيد التطوير من قبل Agent A.');
    } finally {
          ...
          <header className="mb-10 flex justify-between items-center bg-luxury-surface p-6 rounded-3xl border border-white/5">
            ...
            <div className="flex gap-4 items-center">
              ...
              <button 
                ...
                className="bg-purple-600 hover:bg-purple-700 disabled:bg-purple-900 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-purple-500/20 transition-all"
              >
                <Download size={18} />
                {isDownloading ? 'جاري التصدير...' : 'تقرير الإجازات'}
              </button>
            </div>
          </header>
