...
const AdminDashboard = () => {
  ...
  const cpuHistory = [
    ...
    { time: t('adminDashboard.now'), usage: parseInt((metrics?.cpu ?? '24%').replace('%', '')) || 24 },
  ];

  const handleBackup = async () => {
    try {
      const res = await triggerBackup();
      alert(res.data.status || t('adminDashboard.backupSuccess'));
  ...
  const handleClearLogs = async () => {
    if (!window.confirm(t('adminDashboard.confirmClearLogs'))) return;
    try {
      await clearSystemLogs();
      alert(t('adminDashboard.logsCleared'));
      await queryClient.invalidateQueries({ queryKey: queryKeys.admin.logsRoot });
    } catch {
      alert(t('common.error'));
    }
  ...
  return (
    <>
      <header className="mb-10 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight arabic-text">{t('adminDashboard.title')}</h1>
          <p className="text-slate-400 mt-1">{t('adminDashboard.subtitle')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-center mb-10">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold text-white">{t('adminDashboard.systemHealth')}</h2>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              <div className="md:col-span-1 space-y-6">
                <div className="space-y-2">
                  <p className="text-slate-400 text-xs font-bold uppercase tracking-wider flex items-center gap-2"><Cpu size={14}/> {t('adminDashboard.cpuUsage')}</p>
                ...
                <div className="space-y-2">
                  <p className="text-slate-400 text-xs font-bold uppercase tracking-wider flex items-center gap-2"><HardDrive size={14}/> {t('adminDashboard.storage')}</p>
                ...
                <div className="space-y-2">
                  <p className="text-slate-400 text-xs font-bold uppercase tracking-wider flex items-center gap-2"><RefreshCw size={14}/> {t('adminDashboard.uptime')}</p>
            ...
            <div className="mt-12 flex gap-4">
              <button onClick={() => setShowRawLogs(true)} className="bg-white/5 hover:bg-white/10 px-6 py-3 rounded-xl font-bold transition-all border border-white/5 flex items-center gap-2 text-white">
                <Terminal size={18} /> {t('adminDashboard.viewRawLogs')}
              </button>
              <button onClick={() => navigate('/admin/devices')} className="bg-blue-600 hover:bg-blue-700 px-6 py-3 rounded-xl font-bold transition-all shadow-lg shadow-blue-900/20 text-white flex items-center gap-2">
                <Server size={18} /> {t('adminDashboard.manageDevices')}
              </button>
            </div>
          </div>
        </motion.div>
        ...
        >
          <h3 className="font-bold text-lg mb-6 flex items-center gap-2 text-white">
            <Lock className="text-slate-500" size={20} /> {t('adminDashboard.nfcReaders')}
          </h3>
          ...
          <button onClick={() => navigate('/admin/devices')} className="w-full mt-6 py-3 rounded-xl border border-white/10 text-slate-400 font-bold text-sm hover:bg-white/5 hover:text-white transition-all">
            {t('adminDashboard.manageAllDevices')}
          </button>
          <PaginationControls
            page={devicesPage}
      ...
      <section id="audit-table" className="bg-luxury-surface rounded-[2.5rem] shadow-sm border border-white/5 overflow-hidden">
        <div className="p-8 border-b border-white/5 flex justify-between items-center">
          <h3 className="font-bold text-xl flex items-center gap-2 text-white">
            <ShieldAlert className="text-red-500" size={24} /> {t('adminDashboard.auditLogs')}
          </h3>
          <button onClick={handleClearLogs} className="text-xs font-bold text-blue-400 uppercase tracking-widest hover:text-blue-300">{t('adminDashboard.clearLogs')}</button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-right border-collapse">
            <thead className="bg-white/5 text-slate-400 text-[10px] font-black uppercase tracking-[0.15em]">
              <tr>
                <th className="p-6">{t('adminDashboard.actionEvent')}</th>
                <th className="p-6">{t('adminDashboard.originUser')}</th>
                <th className="p-6">{t('adminDashboard.timestamp')}</th>
                <th className="p-6">{t('common.status')}</th>
                <th className="p-6">{t('adminDashboard.details')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {logs.map(log => (
                <tr key={log.logId} className="hover:bg-white/5 transition-all text-sm">
                  ...
                  <td className="p-6 text-slate-500 font-mono">{new Date(log.timestamp).toLocaleTimeString(i18n.language === 'ar' ? 'ar-EG' : 'en-US', {hour: '2-digit', minute: '2-digit'})}</td>
                  <td className="p-6">
                  ...
                  <td className="p-6">
                    <button 
                      onClick={() => alert(`${t('adminDashboard.logDetails')} #${log.logId}:\n${t('adminDashboard.actionEvent')}: ${log.action}\n${t('adminDashboard.originUser')}: ${log.originUser}\n${t('adminDashboard.timestamp')}: ${new Date(log.timestamp).toLocaleString(i18n.language === 'ar' ? 'ar-EG' : 'en-US')}\n${t('common.status')}: ${log.status}`)}
